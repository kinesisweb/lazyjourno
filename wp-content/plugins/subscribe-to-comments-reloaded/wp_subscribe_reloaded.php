		...
		function subscribe_reloaded_show($submit_field = '') {
			...
			if ( $show_subscription_box ) {
				...
				if ( get_option( 'subscribe_reloaded_enable_advanced_subscriptions', 'no' ) == 'no' ) {
					...
					$checkbox_field = "<div class='input-group mb-3'><select name='subscribe-reloaded' id='subscribe-reloaded' class='custom-select form-control'>
								<option value='none' " . ( ( get_option( 'subscribe_reloaded_default_subscription_type' ) === '0' ) ? "selected='selected'" : '' ) . ">Don't subscribe</option>
								<option value='yes' " . ( ( get_option( 'subscribe_reloaded_default_subscription_type' ) === '1' ) ? "selected='selected'" : '' ) . ">Subscribe to all new comments</option>
								<option value='replies' " . ( ( get_option( 'subscribe_reloaded_default_subscription_type' ) === '2' ) ? "selected='selected'" : '' ) . ">Subscribe to my comment replies only</option>
							</select><div class='input-group-append'><span class='input-group-text'>Notify me of new comments by email.</span></div></div>";
				}
				if ( empty( $checkbox_html_wrap ) ) {
					$html_to_show = "$checkbox_field" . $html_to_show;
				} else {
