...
class es_cls_registerhook {
	...
	public static function es_adminmenu() {
		...
		add_menu_page( __( 'LJ Subscribers', 'email-subscribers' ),
			__( 'LJ Subscribers', 'email-subscribers' ), 'edit_posts', 'es-view-subscribers', array( 'es_cls_intermediate', 'es_subscribers' ), 'dashicons-email', 51 );
		...
		add_submenu_page( 'es-view-subscribers', __( 'Weekly Digest', 'email-subscribers' ),
			__( 'Weekly Digest', 'email-subscribers' ), $es_roles_notification, 'es-digest', array( 'es_cls_intermediate', 'es_digest' ) );
	...
	public static function add_optin_optout_link() {
		echo '<tr class="es-signup-confirmation hidden"><td><span  class="es_upsale">Customise confirmation and unsubscribe page <a href="https://www.icegram.com/documentation/how-to-change-simple-unsubscribe-confirmation-message-with-some-beautiful-design-page/?utm_source=es&utm_medium=in_app&utm_campaign=es_upsale" target="_blank">Learn more</a></span></td></tr>';
	}
	...
	public static function add_cron_service() {
		...
		if ( $screen->id === 'email-subscribers_page_es-settings' ) {
            ...
            <tr class="es-cron hidden">
                <td><span class="es_upsale"><?php _e( 'Set automatic cron service <a href="https://www.icegram.com/documentation/how-to-enable-automatic-cron-in-es/?utm_source=es&utm_medium=in_app&utm_campaign=es_upsale" target="_blank">Learn more</a>' ) ?></span></td>
            </tr>
			<?php
		} elseif ( in_array( $screen->id, array( 'email-subscribers_page_es-notification', 'email-subscribers_page_es-sendemail' ), true ) ) {
			?>
            <span class="es_upsale"><?php _e( 'Set automatic cron service <a href="https://www.icegram.com/documentation/how-to-enable-automatic-cron-in-es/?utm_source=es&utm_medium=in_app&utm_campaign=es_upsale" target="_blank">Learn more</a>' ) ?></span>
			<?php
		}
	}
			...
			switch ( $_GET['page'] ) {
				case 'es-view-subscribers':
					...
					$es_select_params = array(
						...
						'es_subscriber_csv_file'       => _x( 'Please select only a csv file. Please check official website for csv structure..', 'view-subscriber-enhanced-select', 'email-subscribers' )
					);
	...
	public static function es_load_widget_scripts_styles() {
		...
		$es_select_params = array(
			...
			'es_rate_limit_notice' => _x( 'You need to wait for some time before subscribing again', 'widget-page-enhanced-select', 'email-subscribers' ),
			'es_success_message'   => _x( 'Successfully Subscribed.', 'widget-page-enhanced-select', 'email-subscribers' ),
			'es_success_notice'    => _x( 'Your subscription was successful! Please check your mailbox and confirm your subscription. If you don\'t see the email within a few minutes, check your spam/junk folder.', 'widget-page-enhanced-select', 'email-subscribers' ),
			...
			'es_recaptcha_failed'  => _x( 'Failed to verify via Recaptcha', 'widget-page-enhanced-select', 'email-subscribers' ),
			'es_try_later'         => _x( 'Please wait and try again', 'widget-page-enhanced-select', 'email-subscribers' ),
	...
	public static function es_add_admin_notices() {
		...
		if ( $es_redesign != 'no' ) {
			$es_new_version_text = __('Email Subscribers plugin redesign:  <a target="_blank" href="https://www.icegram.com/email-subscribers-plugin-redesign/?utm_source=in_app&utm_medium=es_banner&utm_campaign=es_redesign">Check out all exciting changes (February 2019 update)</a>', 'email-subscribers');
			echo '<div class="notice notice-warning" style="background-color: #FFF;"><p style="letter-spacing: 0.6px;">' . $es_new_version_text . ' <a style="float:right" class="es-admin-btn es-admin-btn-secondary" href="?dismiss_admin_notice=1&option_name=es_redesign">' . __( 'No thank you', 'email-subscribers' ) . '</a></p></div>';
		}
		//new version :end
	...
	public static function es_get_form( $instance ) {
        ...
        <div class="es_form_container">
            <form class="es_<?php echo $es_pre ?>_form" data-es_form_id="es_<?php echo $es_pre ?>_form">
				...
				<?php } ?>
				<?php if ( $es_name == "YES" ) { ?>
                    <div class="es_lablebox input-group mb-3">
                    	<div class="input-group-prepend">
                        <span class="es_<?php echo $es_pre ?>_form_name input-group-text"><?php echo __( 'Name', 'email-subscribers' ); ?></span>
                    </div>
                    <input type="text" id="es_txt_name" class="form-control es_textbox_class" name="es_txt_name" value="" maxlength="60" required>
                    </div>
				<?php } ?>
                <div class="es_lablebox input-group mb-3">
                	<div class="input-group-prepend">
                    <span class="input-group-text es_<?php echo $es_pre ?>_form_email"><?php echo __( 'Email', 'email-subscribers' ); ?></span>
                </div>
                    <input type="email" id="es_txt_email" class="form-control es_textbox_class" name="es_txt_email" value="" maxlength="60" required>
                </div>
                <div class="gdpr_message"><span>By clicking subscribe, you are consenting to Lazy Journo storing, and using both your name and email address to send you site news and information. While advertising messaging may occasionally make up a small proportion of these emails, your details will not be sold or transferred to any third parties. You will be asked to confirm your subscription via a link emailed to the address you provided, and unsubscribe links will thereafter be provided as part of every communication from Lazy Journo.</span></div>
                <div class="es_button">
                <button type="submit" id="es_txt_button" name="es_txt_button" class="btn btn-primary"><?php echo __( 'Subscribe', 'email-subscribers' ); ?></button>
                </div>
